...
module.exports.run = async (bot, message, args) => {
  ...
  //!warn @daeshan <reason>
  if(!message.member.hasPermission("MANAGE_MEMBERS")) return message.reply("No can do pleb.");
  let wUser = message.guild.member(message.mentions.users.first()) || message.guild.members.get(args[0])
  if(!wUser) return message.reply("Couldn't find specified user.");
  if(wUser.hasPermission("MANAGE_MESSAGES")) return message.reply("User cannot be warned.");
  ...
  if(warns[wUser.id].warns == 2){
    ...
    if(!muterole) return message.reply("Role does not exist in database");
